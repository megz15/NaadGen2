            ...
            <div id="playbackControls" class="relative flex grow gap-1 p-4 pt-5 bg-[#1d2230b9] rounded-lg backdrop-blur shadow shadow-black border-2 border-gray-400 flex-col max-sm:w-full opacity-{isPlaybackStopped ? 100 : 10} pointer-events-{isPlaybackStopped ? 'auto' : 'none'}">
                ...
                <div>
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <input type="number" bind:value={currBaseFreq} on:change={() => freqObject = genSaptakFreq(shrutis, currBaseFreq)} class="w-20 bg-gray-50 border-2 text-black text-sm rounded-lg px-2 py-1"/>
                    </div>
        ...
        <div class="flex flex-col gap-1">
            {#each currentBandishSectionSvaras[noteModalNoteIndex] as svaras, i}
                <div class="flex justify-between gap-1">
                    <button class="text-lg text-white w-10 bg-orange-500 font-medium rounded-lg py-2.5 hover:scale-105 active:scale-90 hover:shadow-orange-500/50 hover:shadow-[0_0_20px_5px] transition-all duration-200 active:duration-50 active:shadow-orange-500/50 active:shadow-[0_0_20px_5px]" on:click={() => {
                        if (!["S", "P"].includes(svaras[0])) {
                            svaras[0] = svaras[0].toLowerCase() === svaras[0] ? svaras[0].toUpperCase() : svaras[0].toLowerCase()
                        }
                    }}>🔁</button>
                    ...
                    <button class="text-lg text-white w-10 bg-red-500 font-medium rounded-lg py-2.5 hover:scale-105 active:scale-90 hover:shadow-red-500/50 hover:shadow-[0_0_20px_5px] transition-all duration-200 active:duration-50 active:shadow-red-500/50 active:shadow-[0_0_20px_5px]" on:click={() => {
